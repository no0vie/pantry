...
import { List, Tooltip, Button, Popconfirm, Checkbox, Space, Tag, Typography } from 'antd';
...
const ShoppingListItem: React.FC<ShoppingListItemProps> = ({ item, onEdit, onDelete, onToggle }) => (
  <List.Item
    actions={[
      <Tooltip title="Редактировать" key="edit">
        <Button type="text" icon={<EditOutlined />} onClick={() => onEdit(item)} />
      </Tooltip>,
      <Popconfirm title="Удалить пункт?" onConfirm={() => onDelete(item.id)} okText="Да" cancelText="Нет" key="delete">
      ...
      description={
        <Space size={4} wrap>
          ...
          {item.tags.map(tagId => {
            const tag = AVAILABLE_TAGS.find(t => t.id === tagId);
            return tag ? (
              <Tag key={tagId} color={tag.color} icon={getTagIcon(tagId)} style={{ margin: '2px' }}>
                {tag.name}
              </Tag>
            ) : null;
          })}
